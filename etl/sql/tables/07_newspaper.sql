  ...
  -- Article Images
  -- Images associated with newspaper articles (player pictures, team logos, uploads)
  CREATE TABLE IF NOT EXISTS article_images (
      image_id SERIAL PRIMARY KEY,
      article_id INTEGER NOT NULL REFERENCES newspaper_articles(article_id) ON DELETE CASCADE,
      image_type VARCHAR(20) NOT NULL CHECK (image_type IN ('player', 'team_logo', 'uploaded')),

      -- For player images (path constructed as: etl/data/images/players/player_{id}.png)
      player_id INTEGER REFERENCES players_core(player_id) ON DELETE CASCADE,
  ...
  CREATE INDEX IF NOT EXISTS idx_article_images_player ON article_images(player_id) WHERE player_id IS NOT NULL;
  CREATE INDEX IF NOT EXISTS idx_article_images_team ON article_images(team_id) WHERE team_id IS NOT NULL;
